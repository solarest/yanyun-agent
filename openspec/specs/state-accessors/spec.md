...
Provide typed, grouped accessor dataclasses for `AgentState` fields, organized by responsibility domain (control flow, context management, tool execution, task identification). These accessors offer structured read/write operations while maintaining compatibility with the underlying LangGraph `TypedDict` state structure.
