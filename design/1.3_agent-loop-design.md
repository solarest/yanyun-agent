...
| **Observation** | observation_summary, observation_quality | loop_detect | 模式循环检测统计（不再评估工具结果质量） |
...
| 循环检测 | loop_detect | observation_summary/quality, loop_detected=False |
| 循环 | — | 回到 llm_call |
...
**场景 5：纯规划**（planning_only）：

首次/二次重试（count<=2）：
```python
{
    "planning_retry_count": count,
    "messages": [SystemMessage(content="[SYSTEM CORRECTION] You outlined a plan but haven't taken action yet...")],
    "phase": "thinking",
}
```
...
**场景 6：Monologue 检测**（连续 3 轮无工具）：

首次/二次（count<3）：
```python
{
    "stuck_detected": True,
    "stuck_detection_count": count,
    "stuck_type": "monologue",
    "phase": "stuck_recovering",
    "messages": [SystemMessage(content="[SYSTEM CORRECTION] You have been providing analysis without taking action...")],
}
```
...
```python
{
    ...
    "stuck_type": "monologue",
    "phase": "stuck_recovering",
    "error": "Stuck detected (monologue), unrecoverable",
    "should_end": True,
}
```
...
**1. LLM 文本输出评估**（_evaluate_llm_text）：

**优先级链**：
```python
def _evaluate_llm_text(text):
    # 1. 空响应
    ...
    if has_completion_keywords(text):
        has_substance = len(text) > 100 and has_action_keywords(text)
        if has_substance:
    ...
    # 3. 用户提问
    if text.endswith(("?", "？")) or has_question_keywords(text):
        return {"category": "user_question", "route": "complete"}
    
    # 4. 纯规划
    if has_planning_keywords(text) and not has_action_keywords(text):
...
```python
has_substance = (
    len(text.strip()) > 100 and  # 长度阈值
    any(kw in text for kw in ["文件", "file", "创建", "created", 
                               "修改", "modified", "删除", "deleted", 
                               "实现", "implemented"])  # 包含操作描述
)
```

**2. Monologue 模式检测**（_detect_monologue）：

**检测逻辑**：
```python
# 提取最近 3 轮 assistant 消息
...
**关键设计**：
- 检查连续 3 轮（不是累计），确保 LLM 确实卡住
- 只在 substantive_text 路径执行，避免与其他场景冲突
