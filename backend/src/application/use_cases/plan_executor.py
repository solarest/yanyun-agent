...
class PlanExecutor:
    ...
    async def execute_plan(
        ...
        send_message_use_case = main_config["configurable"].get(
    ...
    def _summarize_results(self, plan: Plan, step_results: Dict[int, Dict[str, Any]]) -> str:
        ...
        completed_steps = sum(1 for r in step_results.values()
                              if r["status"] == "completed")
        failed_steps = sum(1 for r in step_results.values()
                           if r["status"] == "failed")
        ...
        for step_id in sorted(step_results.keys()):
            ...
            summary_parts.append(
                f"{status_icon} **Step {step_id}**: {description}")
        ...
        if failed_steps > 0:
            summary_parts.append(
                f"**Warning**: {failed_steps} step(s) failed.")
        else:
